...
namespace NetPdf
{
    public class AtencionIntegralAdolescente
    {
        ...
        public static void CreateAtencionIntegral(string filePDF,
            List<ProblemaList> problemasList,
             ...
             List<EsoCuidadosPreventivosComentarios> ComentariosCP)
        {
            ...
            try
            {
                ...
                var problemasCronicos = problemasList.FindAll(p => p.i_Tipo == (int)TipoProblema.Cronico);
                ...
                if (problemasCronicos != null && problemasCronicos.Count > 0)
                {
                    ...
                    foreach (var item in problemasCronicos)
                    {
                        ...
                        cell = new PdfPCell(new Phrase(item.d_Fecha.ToShortDateString(), fontColumnValue)) { HorizontalAlignment = iTextSharp.text.Element.ALIGN_CENTER, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = alto_Celda_1 };
                        cells.Add(cell);
                ...
                var problemasAgudos = problemasList.FindAll(p => p.i_Tipo == (int)TipoProblema.Agudo);
                ...
                cells = new List<PdfPCell>()
            {
                ...
                new PdfPCell(new Phrase(ComentariosCP.Count == 0 ? "" : ComentariosCP.Where(x => x.GrupoId == FechasCP[0].Listado.Where(z => z.Nombre == "CADA CONSULTA").FirstOrDefault().Hijos.Where(y => y.Nombre == "FIEBRE EN LOS ÚLTIMOS 15 DIAS").FirstOrDefault().GrupoId && x.ParametroId == FechasCP[0].Listado.Where(z => z.Nombre == "CADA CONSULTA").FirstOrDefault().Hijos.Where(y => y.Nombre == "FIEBRE EN LOS ÚLTIMOS 15 DIAS").FirstOrDefault().ParameterId).FirstOrDefault().Comentario, fontColumnValue)) {Colspan =5,HorizontalAlignment = iTextSharp.text.Element.ALIGN_LEFT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = alto_Celda_1},
                ...
                new PdfPCell(new Phrase(ComentariosCP.Count == 0 ? "" : ComentariosCP.Where(x => x.GrupoId == FechasCP[0].Listado.Where(z => z.Nombre == "CADA CONSULTA").FirstOrDefault().Hijos.Where(y => y.Nombre == "SECRECIÓN O LESIÓN EN GENITALES").FirstOrDefault().GrupoId && x.ParametroId == FechasCP[0].Listado.Where(z => z.Nombre == "CADA CONSULTA").FirstOrDefault().Hijos.Where(y => y.Nombre == "SECRECIÓN O LESIÓN EN GENITALES").FirstOrDefault().ParameterId).FirstOrDefault().Comentario, fontColumnValue)) {Colspan =5,HorizontalAlignment = iTextSharp.text.Element.ALIGN_LEFT, VerticalAlignment = iTextSharp.text.Element.ALIGN_MIDDLE, MinimumHeight = alto_Celda_1 },
